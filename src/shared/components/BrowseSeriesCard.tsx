 ...
 *
 * Spine-based series card component for browse pages.
 * Single source of truth for spine-based series card styling across the app.
 *
 * Moved from features/browse/components/SeriesCard.tsx to shared/components/
 * because it's consumed by both browse and search features.
 * Renamed to BrowseSeriesCard to avoid collision with the fanned-cover SeriesCard
 * already in shared/components/.
 *
 * Variants:
...
import { hashString, SPINE_COLOR_PALETTE, useSpineCacheStore } from '@/shared/spine';
import { fitToBoundingBox } from '@/shared/spine';
...
const SPINE_GAP = 1;
const MAX_SPINES_TOTAL_WIDTH_RATIO = 0.5;
...
export function SeriesCard({
  ...
  const spines: SpineItem[] = useMemo(() => {
    ...
    return result;
  }, [bookIds, maxDots, serverDims, maxSpinesTotalWidth]);
  ...
  if (isGrid) {
    return (
      <Pressable
        ...
        accessibilityLabel={`Open series ${name} by ${author}, ${bookCount} ${bookCount === 1 ? 'book' : 'books'}`}
      >
