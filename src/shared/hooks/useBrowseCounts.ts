/**
 * src/shared/hooks/useBrowseCounts.ts
 *
 * Hook to get counts for browse categories (genres, narrators, series, duration ranges).
 * Duration range definitions shared across browse and search features.
 *
 * Moved from features/browse/hooks/ to shared/hooks/ because it's
...
export function useBrowseCounts(): BrowseCounts {
  const { items, isLoaded, currentLibraryId } = useLibraryCache(useShallow((s) => ({ items: s.items, isLoaded: s.isLoaded, currentLibraryId: s.currentLibraryId })));
