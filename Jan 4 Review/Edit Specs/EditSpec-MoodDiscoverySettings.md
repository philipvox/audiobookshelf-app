# Edit Specification: Mood Discovery & Settings Screens
...
const scanForServers = async () => {
  ...
  try {
    ...
    // Scan first 50 addresses (simplified)
    for (let i = 1; i <= 50; i++) {
      const ip = `${subnet}.${i}`;
...
- [ ] Server scan finds local ABS instances
...
| Server discovery | 3 hours | Medium |
| Testing | 2 hours | - |
