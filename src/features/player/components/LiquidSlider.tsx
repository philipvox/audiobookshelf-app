...
// New cleaner SVG - removed drop shadow filter from SVG (we'll do it in RN)
const THUMB_SVG = `<svg viewBox="40 32 174 116" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<rect x="40" y="32" width="174" height="115.691" rx="57.8457" fill="white" fill-opacity="0.01"/>
...
export function LiquidSlider({
  ...
  const valueToPosition = (val: number, width: number): number => {
    'worklet';
    // Thumb center goes from 0 to width, so thumb itself goes from -THUMB_WIDTH/2 to width - THUMB_WIDTH/2
  ...
  const magnifiedTrackStyle = useAnimatedStyle(() => {
    // Thumb center position in track coordinates
    const thumbCenter = translateX.value + THUMB_WIDTH / 2;
    // Shift magnified content so it aligns with what's under the pill
...
const styles = StyleSheet.create({
  ...
  magnifiedTrackWrapper: {
    position: 'absolute',
    left: 0,
    width: 2000, // Large enough to cover full track when scaled
