...
interface TimelineProgressBarProps {
  ...
  /** Called when scrubbing starts with the initial position */
  onScrubStart?: (initialPosition: number) => void;
  /** Called during scrubbing with delta from initial position (in seconds) */
  onScrubUpdate?: (delta: number) => void;
  /** Called when scrubbing ends */
  onScrubEnd?: () => void;
}

const TimelineProgressBar = React.memo(({ position, duration, chapters, onSeek, bookmarks = [], onScrubStart, onScrubUpdate, onScrubEnd }: TimelineProgressBarProps) => {
  // Get theme colors
  const themeColors = usePlayerColors();

  // Track initial position for delta calculation
  const initialPositionRef = useRef(position);

  // Calculate current chapter info for chapter-wide marker
  const { normalizedProgress, chapterStartX, chapterEndX } = useMemo(() => {
    if (duration <= 0) return { normalizedProgress: 0, chapterStartX: 0, chapterEndX: TIMELINE_WIDTH };

    // If no chapters, treat whole book as one chapter
    if (!chapters.length) {
      return {
        normalizedProgress: position / duration,
        chapterStartX: 0,
        chapterEndX: TIMELINE_WIDTH
      };
    }
    ...
    const normalized = (currentChapterIndex * chapterWidth) + (chapterProgress * chapterWidth);

    // Calculate chapter start and end X positions for the chapter-wide highlight
    const startX = currentChapterIndex * (TIMELINE_WIDTH / chapters.length);
    const endX = (currentChapterIndex + 1) * (TIMELINE_WIDTH / chapters.length);

    return { normalizedProgress: normalized, chapterStartX: startX, chapterEndX: endX };
  ...
  // Scrub callbacks for delta indicator
  const handleScrubStart = useCallback(() => {
    initialPositionRef.current = position;
    onScrubStart?.(position);
  }, [position, onScrubStart]);

  const handleScrubUpdate = useCallback((normalizedX: number) => {
    const currentPosition = normalizedToPosition(normalizedX / TIMELINE_WIDTH);
    const delta = currentPosition - initialPositionRef.current;
    onScrubUpdate?.(delta);
  }, [normalizedToPosition, onScrubUpdate]);
  ...
  const scrubGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      runOnJS(handleScrubUpdate)(newX);
    })
    .onEnd((event) => {
      'worklet';
  ...
  return (
    <GestureDetector gesture={combinedGesture}>
      <View style={timelineStyles.container}>
        ...
        {/* Chapter-wide highlight bar - spans full chapter width */}
        <View
          style={[
            timelineStyles.chapterHighlight,
            {
              left: chapterStartX,
              width: chapterEndX - chapterStartX,
              backgroundColor: themeColors.markerColor + '30', // 30% opacity
            }
          ]}
        />

        {/* Chapter progress fill - from chapter start to marker position */}
        <View
          style={[
            timelineStyles.chapterProgress,
            {
              left: chapterStartX,
              width: Math.max(0, normalizedProgress * TIMELINE_WIDTH - chapterStartX),
        ...
        <ReanimatedAnimated.View style={[timelineStyles.marker, { backgroundColor: themeColors.markerColor, shadowColor: themeColors.markerColor }, markerStyle]}>
          <View style={[timelineStyles.markerInner, { backgroundColor: themeColors.markerColor }]} />
...
const timelineStyles = StyleSheet.create({
  ...
  chapterHighlight: {
    position: 'absolute',
    top: TIMELINE_MARKER_RADIUS - 3, // Center vertically with marker
    height: 6,
    borderRadius: 3,
    zIndex: 1,
  },
...
const ChapterTimelineProgressBar = React.memo(({ position, duration, chapters, onSeek, bookmarks = [], libraryItemId, onScrubStart, onScrubUpdate, onScrubEnd }: TimelineProgressBarProps) => {
  // Get theme colors
  const themeColors = usePlayerColors();

  // Track initial position for delta calculation
  const scrubInitialPositionRef = useRef(position);
  ...
  // Scrub state - defined early so useEffect below can reference it
  ...
  // Scrub state (isDirectScrubbing declared earlier for useEffect dependency)
  ...
  const enterDirectScrub = useCallback(() => {
    ...
    // Track initial position and notify parent for delta indicator
    scrubInitialPositionRef.current = currentPosition;
    onScrubStart?.(currentPosition);
  }, [timelineHaptics, onScrubStart]);
  ...
  }, [handleSeek, timelineHaptics, calculateSnapPosition, onScrubEnd]);
  ...
  // Report scrub delta for UI indicator
  const reportScrubDelta = useCallback((newPositionSec: number) => {
    const delta = newPositionSec - scrubInitialPositionRef.current;
    onScrubUpdate?.(delta);
  }, [onScrubUpdate]);
  ...
  const scrubPanGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      // Report delta for UI indicator
      runOnJS(reportScrubDelta)(newSeconds);
  ...
  return (
    <View style={chapterTimelineStyles.outerContainer}>
      ...
      <View style={[chapterTimelineStyles.markerLine, { backgroundColor: themeColors.markerColor }]} />
      <View style={[chapterTimelineStyles.markerDot, { backgroundColor: themeColors.markerColor }]} />
...
const chapterTimelineStyles = StyleSheet.create({
  ...
  markerLine: {
    ...
    width: 2,
    height: CHAPTER_TICKS_AREA_HEIGHT -50, // Extend up through ticks area
    backgroundColor: '#F50101',
    zIndex: 5,
  },
  markerDot: {
    position: 'absolute',
    bottom: CHAPTER_TICKS_AREA_HEIGHT - 65, // At top of line, centered
    left: CHAPTER_MARKER_X - 12, // Center the 6px dot
    width: 24,
    height: 24,
    borderRadius: 24,
    backgroundColor: '#F50101',
...
export function CDPlayerScreen() {
  ...
  const handleTimelineScrubUpdate = useCallback((delta: number) => {
    ...
    >
      ...
      <View style={styles.backgroundContainer}>
        ...
        <BlurView
          ...
          style={StyleSheet.absoluteFill}
        />
        {/* Gradient feathers to background color at bottom */}
        <LinearGradient
          colors={isDarkMode
            ? ['transparent', 'transparent', 'rgba(0,0,0,0.3)', 'rgba(0,0,0,1)', 'rgba(0,0,0,1)']
            : ['transparent', 'transparent', 'rgba(255,255,255,0.3)', 'rgba(255,255,255,0.7)', '#FFFFFF']}
        ...
        <LinearGradient
          ...
          locations={[0, 0.3, 0.5]}
          style={[StyleSheet.absoluteFill, { bottom: 'auto', height: scale(200) }]}
        />
      </View>
      ...
      <View style={[styles.contentArea, { marginTop: scale(-60) }]}>
        ...
        {/* Cover buttons overlay - cover hidden, background image visible behind */}
        <View style={styles.standardCoverContainerFull}>
            ...
            {isDownloaded ? (
              ...
              >
                <Download size={scale(18)} color="#FFFFFF" strokeWidth={2.5} />
              </TouchableOpacity>
            )}
            ...
            {/* Top-right: Settings */}
            <TouchableOpacity
              style={styles.coverOverlayTopRightWrapper}
              onPress={openSettings}
              activeOpacity={0.7}
            >
              <SettingsIconCircle darkPill />
            </TouchableOpacity>

          </View>

        {/* Edge buttons - Queue (left edge) and Bookmark (right edge) */}
        <TouchableOpacity
          onPress={openQueue}
          style={styles.coverEdgeButtonLeft}
          activeOpacity={0.7}
          accessibilityLabel={queueCount > 0 ? `Queue with ${queueCount} items` : 'Queue empty'}
          accessibilityRole="button"
        >
          <Layers size={scale(18)} color="#FFFFFF" strokeWidth={2} />
          {queueCount > 0 && (
        ...
        <TouchableOpacity
          ...
          style={styles.coverEdgeButtonRight}
          activeOpacity={0.7}
        ...
        >
          ...
          >
            {showBookmarkPill ? (
              ...
              <Bookmark size={scale(18)} color="#FFFFFF" strokeWidth={2} />
            )}
          </Animated.View>
        </TouchableOpacity>
        ...
        {/* Current time - centered above timeline marker */}
        <View style={[styles.currentTimeAboveMarker, { bottom: insets.bottom + scale(200) }]}>
          {/* Seeking delta indicator - show for button skips or timeline scrubbing */}
          {seekingState.isActive && (
            <View style={styles.seekingDeltaContainer}>
              <Text style={[styles.seekingDeltaText, { color: themeColors.accent }]}>
                {seekingState.direction === 'back' ? '−' : '+'}{seekingState.amount}s
              </Text>
            </View>
          )}
          {timelineScrubDelta !== null && !seekingState.isActive && (
            <View style={styles.seekingDeltaContainer}>
              <Text style={[styles.seekingDeltaText, { color: themeColors.accent }]}>
                {timelineScrubDelta >= 0 ? '+' : '−'}
                {Math.abs(timelineScrubDelta) >= 60
                  ? `${Math.floor(Math.abs(timelineScrubDelta) / 60)}:${String(Math.floor(Math.abs(timelineScrubDelta) % 60)).padStart(2, '0')}`
                  : `${Math.floor(Math.abs(timelineScrubDelta))}s`
                }
              </Text>
            </View>
          )}
          <Text style={[styles.currentTimeText, { color: themeColors.textPrimary }]}>
            {formattedPosition}
          </Text>
        </View>

        {/* Progress Bar */}
        <View style={[styles.progressWrapper, styles.progressWrapperStandard, { bottom: insets.bottom + scale(120) }]}>
          {progressMode === 'book' ? (
        ...
        {/* Chapter (left) and Duration (right) - below timeline */}
        <View style={[styles.chapterDurationRow, { bottom: insets.bottom + scale(95) }]}>
          <TouchableOpacity onPress={openChapters} style={styles.chapterTouchLeft}>
            <Text style={[styles.chapterTextBelow, { color: themeColors.textSecondary }]} numberOfLines={1}>
              {currentNormalizedChapter?.displayTitle || `Chapter ${chapterIndex + 1}`}
            </Text>
          </TouchableOpacity>
          <Text style={[styles.durationTextRight, { color: themeColors.textSecondary }]}>
            {formattedDuration}
          </Text>
        </View>
        ...
        <View style={[styles.standardControlsBar, { backgroundColor: themeColors.buttonBackground, bottom: insets.bottom }]}>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <PrevChapterIcon color={themeColors.buttonText} />
            </TouchableOpacity>
                ...
                <View style={styles.sleepTimerControl}>
                  ...
                  {isPlaying ? (
                    <Svg width={scale(14)} height={scale(14)} viewBox="0 0 24 24" fill="none">
                      <Rect x="6" y="5" width="4" height="14" fill={themeColors.accent} />
                      <Rect x="14" y="5" width="4" height="14" fill={themeColors.accent} />
                    </Svg>
                  ) : (
                    <Svg width={scale(14)} height={scale(14)} viewBox="0 0 24 24" fill="none">
                      <Path
                  ...
                  {/* Time remaining in accent bold - live seconds countdown */}
                  <Text style={[styles.sleepTimerText, { color: themeColors.accent }]}>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <NextChapterIcon color={themeColors.buttonText} />
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  standardTitleSection: {
    position: 'absolute',
    top: scale(450), // Below cover with extra space
    left: 0,
    right: 0,
    alignItems: 'center',
  ...
  // Cover overlay positions for top icons - aligned with edge buttons
  coverOverlayTopLeft: {
    position: 'absolute',
    top: scale(0),
    left: -scale(60), // Aligned with left edge button
    width: scale(36),
    height: scale(36),
    borderRadius: scale(18),
    borderWidth: 1,
  ...
  coverOverlayTopRight: {
    position: 'absolute',
    top: scale(0),
    right: -scale(18), // Aligned with right edge button
    width: scale(36),
    height: scale(36),
    borderRadius: scale(18),
    borderWidth: 1,
  ...
  coverOverlayTopRightWrapper: {
    position: 'absolute',
    top: scale(0),
    right: -scale(60), // Aligned with right edge button
    zIndex: 25,
  },
  ...
  coverEdgeButtonLeft: {
    position: 'absolute',
    top: scale(190) + COVER_SIZE * 0.55,
    left: (wp(80) - COVER_SIZE) / 2 - scale(18), // Half button overlaps edge
    width: scale(36),
    height: scale(36),
    borderRadius: scale(18),
    borderWidth: 1,
  ...
  coverEdgeButtonRight: {
    position: 'absolute',
    top: scale(190) + COVER_SIZE * 0.55,
    left: (wp(120) - COVER_SIZE) / 2 + COVER_SIZE - scale(18), // Half button overlaps edge
    width: scale(36),
    height: scale(36),
    borderRadius: scale(18),
    borderWidth: 1,
