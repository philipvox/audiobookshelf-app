# Edit Specification: Series Screens (v2)

**Version:** 2 (post-validation)
**Changes from v1:** Fixed calculateSeriesProgress → useSeriesProgress, corrected effort estimate

**Covers Action Plan Items:** 1.7, 4.7, 4.19
**Priority:** Critical (1.7) / Medium (4.7, 4.19)
**Effort:** S (Small) - 3-5 hours
...
|-----------|------------|
| 4.7 Series filter | 2.10 useSeriesProgress (from PatternConsolidation) |
| Type fixes | 2.5 getBookMetadata |
...
### 4.7: Add Series Filter by Status
...
**Filter logic using useSeriesProgress:**
```typescript
// Helper component that uses the hook
const SeriesFilteredList: React.FC<{ series: SeriesInfo[]; filter: SeriesFilter }> = ({
  series,
  filter,
}) => {
  const filteredSeries = useMemo(() => {
    ...
    return series.filter(s => {
      const books = getSeriesBooks(s.name);
      // Use the hook for each series
      return matchesFilter(books, filter);
    });
  }, [series, filter]);

  return <SectionList sections={groupByLetter(filteredSeries)} ... />;
};

// Filter matching function (can be inline or extracted)
function useSeriesFilterMatch(seriesBooks: LibraryItem[], filter: SeriesFilter): boolean {
  const { completedBooks, inProgressBooks, totalBooks } = useSeriesProgress(seriesBooks);

  switch (filter) {
    case 'completed':
      return completedBooks === totalBooks && totalBooks > 0;
    case 'in-progress':
...
### 4.19: Make Bell Icon More Prominent
...
**Use getBookMetadata helper after 2.5:**
```typescript
...
| useSeriesProgress | Depends on 2.10 completion |
...
| Implement filter logic with useSeriesProgress | 1.5 hours | Low |
